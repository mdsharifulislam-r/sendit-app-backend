...
export class ReviewService {
    ...
    async create(createReviewDto: CreateReviewDto) {
        ...
        if (review.type == ReviewType.PLATFORM) {
            ...
            await this.snsService.publish<{ userId: string, tripId?: string }>('review.calculate', { userId: review.transporter.toString(), tripId: review?.trip?.toString() })
        }
    ...
    async calculateAvgRatingAndSave(payload: { userId: string, tripId?: string }) {
        ...
        const tripAvgRating = await this.reviewModel.aggregate([
            {
                $match: {
                    trip: new mongoose.Types.ObjectId(payload.tripId),
                },
            },
            {
                $group: {
        ...
        await this.tripModel.findByIdAndUpdate(payload.tripId, {
            avg_rating: tripAvgRating?.length > 0 ? tripAvgRating[0]?.avgRating : 0,
            total_reviews: tripAvgRating?.length > 0 ? tripAvgRating[0]?.reviewCount : 0,
        })
    }
