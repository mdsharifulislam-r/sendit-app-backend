...
import { Transaction, TRANSACTION_PAYMENT_TYPE, TRANSACTION_STATUS, TRANSACTION_TYPE, TransactionDocument } from '../transaction/transaction.entity';
...
export class WalletService {
    ...
    async getWalletStats(userId: string) {
        ...
        thisMonthEndDate.setDate(31);
        const thisMonthStats = await this.transactionModel.aggregate([
            {
                $match: {
        ...
        const totalWithdraw = thisMonthStats[0]?.total || 0;

        const upcommingEarnings = await this.bookingModel.aggregate([
            {
                $match: {
                    transporter: new Types.ObjectId(userId),
            ...
            },
            {
                $group: {
                    _id: null,
                    total: { $sum: "$price_breakdown.subtotal" },
                },
            },
        ]);

        const upcommingEarningsAmount = upcommingEarnings[0]?.total || 0;

        return sendResponse({
            statusCode: HttpStatus.OK,
            message: 'Wallet stats',
            data: {
                totalWithdraw: Number((totalWithdraw).toFixed(2)),
                upcommingEarnings: Number((upcommingEarningsAmount).toFixed(2)),
            },
            success: true,
        });
