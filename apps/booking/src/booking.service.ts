...
export class BookingService {
  ...
  async placeBooking(userId: string, session_id: string, trip_id: string, coupon?: string, payment_intent_id?: string) {
    ...
    try {
      ...
      await Promise.all([
        ...
        this.snsService.publish('qr.code.generate', { data: JSON.stringify({ id: savedBooking.id, date: new Date(), return_address: savedBooking.pickup_address, pickup_address: savedBooking.dropoff_address }), id: savedBooking.id }),
  ...
  async getBookingRequest(userId: string, query: Record<string, any>) {
    ...
    const [bookings, pagination] = await Promise.all([
      qb.modelQuery.populate('sender', 'name email image').populate('trip', 'return_date departure_date').lean(),
  ...
  async acceptOrRejectBooking(userId: string, bookingId: string, status: BOOKING_STATUS, rejection_reason?: string) {
    ...
    if (status === BOOKING_STATUS.CONFIRMED) {
      ...
      await this.tripModel.findByIdAndUpdate(booking.trip, { $inc: { total_bookings: 1, exist_weight: -booking.weight } })
      await this.cacheService.deleteByPattern(`booking_request:${userId}`)
      await this.cacheService.deleteByPattern(`my_parcel:${booking.sender._id.toString()}`)
      this.snsService.publish('notification.send', {
        title: `Booking #${booking.id}`,
  ...
  async markAsDeliverdParcel(bookingId: string, userId: string, payload: DeliveryConfirmationDto) {
    ...
    if (payload.recipient_signature_type === "image" && payload.recipient_signature) {
      const uploadedImage = await this.s3Service.uploadFile(payload.recipient_signature)
      payload.recipient_signature = uploadedImage?.url
    }
  ...
  async getMyParcel(userId: string, query: Record<string, any>) {
    ...
    const [bookings, pagination] = await Promise.all([
      qb.modelQuery.populate('transporter', 'name email image').populate('trip', 'return_date departure_date').lean(),
      qb.getPaginationInfo()
    ]);
  ...
  async changeStatusByqrcode(id: string, userId: string) {
    const booking = await this.bookingModel.findOne({ id })
    if (!booking) {
      throw new ApiError(404, 'Booking not found')
    }
    if (booking.sender.toString() != userId && booking.transporter.toString() != userId && booking.receiver.toString() != userId) {
      throw new ApiError(403, 'You are not authorized to update this booking')
    }
    if (booking.status != BOOKING_STATUS.CONFIRMED) {
    ...
    if (![TIMELINE_TYPE.BOOKED, TIMELINE_TYPE.IN_TRANSIT].includes(booking.current_stage)) {
